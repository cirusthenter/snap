...
public:
    ...
    {
        if (this == &HashKeyDat || (HashCd == HashKeyDat.HashCd && Key == HashKeyDat.Key && Dat == HashKeyDat.Dat)) {
            return true;
        }
        return false;
    }
...
public:
    ...
    THashKeyDatI(const THashKeyDatI& _HashKeyDatI)
        : KeyDatI(_HashKeyDatI.KeyDatI)
        , EndI(_HashKeyDatI.EndI)
    {
    }
    THashKeyDatI(const THKeyDat* _KeyDatI, const THKeyDat* _EndI)
        : KeyDatI((THKeyDat*)_KeyDatI)
        , EndI((THKeyDat*)_EndI)
    {
    }
    ...
    {
        KeyDatI++;
        while (KeyDatI < EndI && KeyDatI->HashCd == -1) {
            KeyDatI++;
        }
        return *this;
    }
    ...
    {
        Assert((KeyDatI != NULL) && (KeyDatI->HashCd != -1));
        return KeyDatI->Key;
    }
...
private:
    ...
    public:
        ...
        bool operator()(const int& KeyId1, const int& KeyId2) const
        {
            if (CmpKey) {
                if (Asc) {
                    return Hash.GetKey(KeyId1) < Hash.GetKey(KeyId2);
                } else {
                    return Hash.GetKey(KeyId2) < Hash.GetKey(KeyId1);
                }
            } else {
                if (Asc) {
...
public:
    ...
    TIter BegI() const
    {
        ...
        if (IsKeyIdEqKeyN()) {
            return TIter(KeyDatV.BegI(), KeyDatV.EndI());
        }
        int FKeyId = -1;
        FNextKeyId(FKeyId);
...
THash<TKey, TDat, THashFunc>::THash(const int& ExpectVals, const bool& _AutoSizeP)
    : PortV(GetNextPrime(ExpectVals / 2))
    , KeyDatV(ExpectVals, 0)
...
int THash<TKey, TDat, THashFunc>::AddKey(const TKey& Key)
{
    ...
    while ((KeyId != -1) && !((KeyDatV[KeyId].HashCd == HashCd) && (KeyDatV[KeyId].Key == Key))) {
        PrevKeyId = KeyId;
...
void THash<TKey, TDat, THashFunc>::DelKey(const TKey& Key)
{
    ...
    if (PrevKeyId == -1) {
        PortV[PortN] = KeyDatV[KeyId].Next;
    } else {
        KeyDatV[PrevKeyId].Next = KeyDatV[KeyId].Next;
    }
    KeyDatV[KeyId].Next = FFreeKeyId;
    FFreeKeyId = KeyId;
    FreeKeys++;
...
{
    IAssert(!Empty());
    if (FreeKeys / double(Len() + FreeKeys) > EmptyFrac) {
        Defrag();
    }
...
public:
    ...
    PHash<TKey, TDat>(const int& MxVals, const int& Vals)
        : H(MxVals, Vals)
    {
    }
    ...
    PHash<TKey, TDat>(const THash<TKey, TDat>& _V)
        : H(_V)
    {
    }
    ...
    explicit PHash<TKey, TDat>(TSIn& SIn)
        : H(SIn)
    {
    }
...
TStr GetStr(const int& StrId) const
{
    ...
    if (StrId == 0)
        ...
        return TStr(Bf + (TSize)IdOffV[StrId]);
}
...
int Cmp(const int& StrId, const char* Str) const
{
    Assert(StrId < GetStrs());
    if (StrId != 0)
        return strcmp(Bf + (TSize)IdOffV[StrId], Str);
    else
        return strcmp("", Str);
}
...
int GetPrimHashCd(const int& StrId)
{
    Assert(StrId < GetStrs());
    if (StrId != 0)
        return GetPrimHashCd(Bf + (TSize)IdOffV[StrId]);
    else
...
int GetSecHashCd(const int& StrId)
{
    Assert(StrId < GetStrs());
    if (StrId != 0)
        return GetSecHashCd(Bf + (TSize)IdOffV[StrId]);
    else
        return GetSecHashCd("");
}
}
...
public:
    ...
    {
        if (!Hash.Pool.Empty()) {
            Pool = PStringPool(new TStringPool(*Hash.Pool));
        }
    }
    ...
    {
        const int KeyId = GetKeyId(Key);
        if (KeyId != -1) {
            Dat = KeyDatV[KeyId].Dat;
            return true;
        } else
            return false;
    }
    ...
    {
        const int KeyId = GetKeyId(Key.CStr());
        if (KeyId != -1) {
            Dat = KeyDatV[KeyId].Dat;
            return true;
        } else
...
void TStrHash<TDat, TStringPool, THashFunc>::Resize()
{
    ...
    if (PortV.Empty()) {
        PortV.Gen(17);
        PortV.PutAll(-1);
    } else if (AutoSizeP && KeyDatV.Len() > 3 * PortV.Len()) {
...
{
    if (Pool.Empty())
        Pool = TStringPool::New();
    if ((AutoSizeP && KeyDatV.Len() > PortV.Len()) || PortV.Empty())
        Resize();
    const int PortN = abs(THashFunc::GetPrimHashCd(Key) % PortV.Len());
    const int HashCd = abs(THashFunc::GetSecHashCd(Key));
    int PrevKeyId = -1;
