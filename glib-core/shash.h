...
public:
    ...
    static void LoadHash(TSIn& SIn, THash<TKey, TDat, THashFunc>& Hash, int LoadN = -1)
    {
        ...
        if (ElemCnt < LoadN || LoadN == -1) {
            LoadN = ElemCnt;
        }
        printf("Loading %s: %d elements ... ", SIn.GetSNm().CStr(), LoadN);
        Hash.Gen(LoadN);
        for (int i = 0; i < LoadN; i++) {
            Hash.AddDat(TKey(SIn)).Load(SIn);
        }
...
{
    static const int bits_in[256] = {
        // # of bits set in one char
        0,
        1,
        1,
        2,
...
template <class TVal, uint16 GroupSize>
TSparseGroup<TVal, GroupSize>::TSparseGroup(const TSparseGroup& SG)
    : Buckets(SG.Buckets)
    , Group(NULL)
{
...
bool TSparseGroup<TVal, GroupSize>::operator<(const TSparseGroup& SG) const
{
    if (Buckets < SG.Buckets)
        return true;
    if (memcmp(BitSet, SG.BitSet, sizeof(BitSet)) == -1)
        return true;
    for (int b = 0; b < Buckets; b++) {
        if (Group[b] < SG.Group[b])
            return true;
...
TVal& TSparseGroup<TVal, GroupSize>::Set(const int& ValN, const TVal& Val)
{
    ...
    if (!BMTest(ValN)) {
        ...
        for (int b = 0; b < Offset; b++)
            Group[b] = OldGroup[b];
        for (int b = Offset + 1; b <= Buckets; b++)
            Group[b] = OldGroup[b - 1];
        if (OldGroup != NULL)
            delete[] OldGroup;
        Buckets++;
        BMSet(ValN);
...
public:
    ...
    {
        if (CurOff + 1 == GroupI->Len()) {
            CurOff = 0;
            if (GroupI < EndI) {
                GroupI++;
                while (GroupI < EndI && GroupI->Empty()) {
                    GroupI++;
                }
            }
        } else {
    ...
    {
        if (CurOff == 0) {
            while (GroupI >= BegI && GroupI->Empty()) {
                GroupI--;
            }
            if (GroupI >= BegI)
                CurOff = GroupI->Len() - 1;
...
public:
    TSparseTable(const int& MaxVals = 0)
        : MxVals(MaxVals)
        , Vals(0)
        , GroupV(GetGroups(MaxVals), GetGroups(MaxVals))
    {
    }
...
public:
    ...
    explicit TSHashKeyDat(TSIn& SIn)
        : Key(SIn)
        , Dat(SIn)
    {
    }
...
public:
    ...
    void Load(TSIn& SIn)
    {
        ShrinkThresh.Load(SIn);
        ExpandThresh.Load(SIn);
        Table.Load(SIn);
    }
    void Save(TSOut& SOut) const
    {
        ShrinkThresh.Save(SOut);
        ExpandThresh.Save(SOut);
        Table.Save(SOut);
    }
    ...
    {
        Assert(IsKey(Key));
        return Table.GetI(GetKeyId(Key));
    }
    ...
    int GetRndKeyId(TRnd& Rnd = TInt::Rnd) const
    {
        ...
        while (!IsKeyId(KeyId)) {
            KeyId = Rnd.GetUniDevInt(Reserved());
        }
        return KeyId;
    }
...
{
    int Size = MinBuckets;
    while (Size * MxOccupancy < WantedVals || CurVals >= Size * MxOccupancy)
        Size *= 2;
    return Size;
}
...
void TSparseHash<TKey, TDat, GroupSize>::MoveFrom(TSparseHash& HT, const int& MnWanted)
{
    ...
    if (MnWanted == 0)
        ...
        NewSize = GetMinSize(HT.Reserved(), MnWanted);
    if (NewSize > Reserved()) {
        Table.Resize(NewSize);
        ResetThresh();
    }
...
{
    if (Reserved() > MnWanted && Len() + ValsToAdd < ExpandThresh) {
        return;
    }
...
public:
    ...
    {
        Assert(IsKeyId(KeyId));
        return Table.GetI(KeyId);
    }
...
void TSparseSet<TKey, GroupSize>::MoveFrom(TSparseSet& SSet, const int& MnWanted)
{
    ...
    int NewSize;
    if (MnWanted == 0)
        NewSize = SSet.Reserved();
    else
        NewSize = GetMinSize(SSet.Reserved(), MnWanted);
    if (NewSize > Reserved()) {
        Table.Resize(NewSize);
        ResetThresh();
    }
...
public:
    ...
    {
        Assert((KeyI != NULL) && (KeyI->HashCd != -1));
        return KeyI->Key;
    }
};
...
public:
    ...
    THashSet(const THashSet& Set)
        : PortV(Set.PortV)
        , KeyV(Set.KeyV)
        , AutoSizeP(Set.AutoSizeP)
        , FFreeKeyId(Set.FFreeKeyId)
    ...
    TIter BegI() const
    {
        if (Len() > 0) {
            if (IsKeyIdEqKeyN()) {
                return TIter(KeyV.BegI(), KeyV.EndI());
            }
            int FKeyId = -1;
            FNextKeyId(FKeyId);
    ...
    {
        PortV.Gen(GetNextPrime(ExpectVals / 2));
        KeyV.Gen(ExpectVals, 0);
        FFreeKeyId = -1;
        FreeKeys = 0;
        PortV.PutAll(TInt(-1));
    }
    ...
    {
        for (int KeyIdN = 0; KeyIdN < KeyIdV.Len(); KeyIdN++) {
...
THashSet<TKey, THashFunc>::THashSet(const int& ExpectVals, const bool& _AutoSizeP)
    : PortV(GetNextPrime(ExpectVals / 2 + 1))
    , KeyV(ExpectVals, 0)
    , AutoSizeP(_AutoSizeP)
    , FFreeKeyId(-1)
    , FreeKeys(0)
{
    PortV.PutAll(TInt(-1));
...
THashSet<TKey, THashFunc>::THashSet(const TVec<TKey>& _KeyV)
    : PortV(GetNextPrime(_KeyV.Len() / 2 + 1))
    , KeyV(_KeyV.Len(), 0)
    , AutoSizeP(false)
    , FFreeKeyId(-1)
    , FreeKeys(0)
...
{
    if ((KeyV.Len() > 2 * PortV.Len()) || PortV.Empty()) {
        Resize();
    }
    ...
    while ((KeyId != -1) && !((KeyV[KeyId].HashCd == HashCd) && (KeyV[KeyId].Key == Key))) {
        PrevKeyId = KeyId;
    ...
    if (KeyId == -1) {
        if (FFreeKeyId == -1) {
            ...
            KeyId = FFreeKeyId;
            FFreeKeyId = KeyV[FFreeKeyId].Next;
            FreeKeys--;
            KeyV[KeyId].Next = -1;
...
void THashSet<TKey, THashFunc>::DelKey(const TKey& Key)
{
    ...
    if (PrevKeyId == -1) {
        PortV[PortN] = KeyV[KeyId].Next;
    } else {
        KeyV[PrevKeyId].Next = KeyV[KeyId].Next;
    }
    KeyV[KeyId].Next = FFreeKeyId;
    FFreeKeyId = KeyId;
    FreeKeys++;
...
public:
    ...
    explicit TPackVec(const int& _Vals)
        : Vals(_Vals)
    {
        if (Vals == 0) {
            ValT = NULL;
    ...
    void AddV(TSIn& FIn)
    {
        ...
        FIn.Load(NVals);
        ResizeDelta(NVals);
        FIn.LoadBf(ValT + Vals - NVals, sizeof(TVal) * NVals);
    }

    void Sort(const bool& Asc = true)
    {
        if (Asc) {
            TVec<TVal>::QSortCmp(BegI(), EndI(), TLss<TVal>());
        } else {
            TVec<TVal>::QSortCmp(BegI(), EndI(), TGtr<TVal>());
        }
    }
};
