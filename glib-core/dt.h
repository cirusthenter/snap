...
public:
    ...
    {
        Bf = new char[(MxBfL = _MxBfL) + 1];
        Bf[BfL = 0] = 0;
    }
    TChA(const char* CStr)
    {
        Bf = new char[(MxBfL = BfL = int(strlen(CStr))) + 1];
        strcpy(Bf, CStr);
    }
    TChA(const char* CStr, const int& StrLen)
        ...
        , BfL(StrLen)
    {
        Bf = new char[StrLen + 1];
        strncpy(Bf, CStr, StrLen);
        Bf[StrLen] = 0;
    }
    TChA(const TChA& ChA)
    {
        Bf = new char[(MxBfL = ChA.MxBfL) + 1];
        BfL = ChA.BfL;
        strcpy(Bf, ChA.CStr());
    }
    TChA(const TStr& Str);
    TChA(const TMem& Mem)
    {
        Bf = new char[(MxBfL = BfL = Mem.Len()) + 1];
        Bf[MxBfL] = 0;
    ...
    { // J:
        SOut.Save(SaveCompact ? BfL : MxBfL);
        SOut.Save(BfL);
        SOut.Save(Bf, BfL);
    }
    void LoadXml(const PXmlTok& XmlTok, const TStr& Nm);
...
public:
    ...
    TRStr(const char* CStr1, const char* CStr2)
    {
        ...
        Bf = new char[CStr1Len + int(strlen(CStr2)) + 1];
        strcpy(Bf, CStr1);
        strcpy(Bf + CStr1Len, CStr2);
    }
    TRStr(const char& Ch)
    {
        Refs = 0;
    ...
    {
        static TRStr NullRStr;
        Assert(NullRStr.Bf != NULL);
        return &NullRStr;
    }
};
...
public:
    ...
    TStr(const TChA& ChA)
    {
        RStr = GetRStr(ChA.CStr());
        RStr->MkRef();
    }
    TStr(const TSStr& SStr)
    {
        RStr = GetRStr(SStr.CStr());
        RStr->MkRef();
    }
    ...
    {
        RStr = GetChStr(Ch).RStr;
        RStr->MkRef();
    }
    TStr(const TMem& Mem)
    {
        RStr = new TRStr(Mem.Len());
        RStr->MkRef();
        memcpy(CStr(), Mem(), Mem.Len());
        Optimize();
    }
    ...
    {
        int SInLen = SIn->Len();
        RStr = new TRStr(SInLen);
        RStr->MkRef();
        SIn->GetBf(CStr(), SInLen);
        Optimize();
    }
    ~TStr() { RStr->UnRef(); }
    explicit TStr(TSIn& SIn, const bool& IsSmall = false)
        : RStr(new TRStr(SIn, IsSmall))
    {
        RStr->MkRef();
        Optimize();
    }
    ...
    {
        int Val;
        IAssertR(IsInt(false, 0, 0, Val), *this);
        return Val;
    }
    int GetInt(const int& DfVal) const
    {
        int Val;
        if (IsInt(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
    ...
    {
        uint Val;
        IAssert(IsUInt(false, 0, 0, Val));
        return Val;
    }
    uint GetUInt(const uint& DfVal) const
    {
        uint Val;
        if (IsUInt(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
    ...
    {
        int64 Val;
        IAssert(IsInt64(false, 0, 0, Val));
        return Val;
    }
    int64 GetInt64(const int64& DfVal) const
    {
        int64 Val;
        if (IsInt64(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
    ...
    {
        uint64 Val;
        IAssert(IsUInt64(false, 0, 0, Val));
        return Val;
    }
    uint64 GetUInt64(const uint64& DfVal) const
    {
        uint64 Val;
        if (IsUInt64(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
    ...
    {
        int Val;
        IAssert(IsHexInt(false, 0, 0, Val));
        return Val;
    }
    int GetHexInt(const int& DfVal) const
    {
        int Val;
        if (IsHexInt(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
    ...
    {
        int64 Val;
        IAssert(IsHexInt64(false, 0, 0, Val));
        return Val;
    }
    int64 GetHexInt64(const int64& DfVal) const
    {
        int64 Val;
        if (IsHexInt64(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
    ...
    {
        double Val;
        IAssert(IsFlt(false, 0, 0, Val));
        return Val;
    }
    double GetFlt(const double& DfVal) const
    {
        double Val;
        if (IsFlt(false, 0, 0, Val)) {
            return Val;
        } else {
            return DfVal;
        }
...
TStrPool(const TStrPool& Pool)
    ...
    , GrowBy(Pool.GrowBy)
{
    Bf = (char*)malloc(Pool.MxBfL);
    IAssertR(Bf, TStr::Fmt("Can not resize buffer to %u bytes. [Program failed to allocate more memory. Solution: Get a bigger machine.]", MxBfL).CStr());
    memcpy(Bf, Pool.Bf, Pool.BfL);
}
~TStrPool()
{
    if (Bf)
...
public:
    ...
    {
        if (Val > 100 * 1000) {
            return GetStr(Val / 1000) + "K";
        } else if (Val > 1000) {
            return GetStr(Val / 1000) + "." + GetStr((Val % 1000) / 100) + "K";
        } else {
            return GetStr(Val);
        }
    }
...
public:
    ...
    static bool IsHex(const char& Ch) { return (('0' <= Ch) && (Ch <= '9')) || (('A' <= Ch) && (Ch <= 'F')) || (('a' <= Ch) && (Ch <= 'f')); }
...
public:
    ...
    {
        uint Ip;
        return IsIpStr(IpStr, Ip, SplitCh);
    }
...
public:
    ...
    explicit TUInt64(void* Pt)
        : Val(0)
    {
        TConv_Pt64Ints32 Conv(Pt);
        Val = Conv.GetUInt64();
    }
...
public:
    ...
    {
        int Expn;
        return int((frexp(Val, &Expn) - 0.5) * double(TInt::Mx));
    }
    int GetSecHashCd() const
    {
        int Expn;
    ...
    {
        if (fabs(Val) > 100 * 1000) {
            return TStr::Fmt("%.0fK", Val / 1000);
        } else if (fabs(Val) > 1000) {
            return TStr::Fmt("%.1fK", Val / 1000);
        } else {
            return TStr::Fmt("%.0f", Val);
        }
    }
    static TStr GetMegaStr(const double& Val)
    {
        if (fabs(Val) > 100 * 1000000) {
            return TStr::Fmt("%.0fM", Val / 1000000);
        } else if (fabs(Val) > 1000000) {
            return TStr::Fmt("%.1fM", Val / 1000000);
        } else {
            return GetKiloStr(Val);
        }
    }
    ...
    {
        if (fabs(Val) > 100 * 1000000000.0) {
            return TStr::Fmt("%.0fG", Val / 1000000000.0);
        } else if (fabs(Val) > 1000000000.0) {
            return TStr::Fmt("%.1fG", Val / 1000000000.0);
        } else {
            return GetMegaStr(Val);
        }
    }
};
