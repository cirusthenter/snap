...
public:
    ...
    bool operator<(const TTriple& Triple) const
    {
        return (Val1 < Triple.Val1) || ((Val1 == Triple.Val1) && (Val2 < Triple.Val2)) || ((Val1 == Triple.Val1) && (Val2 == Triple.Val2) && (Val3 < Triple.Val3));
    }
...
public:
    ...
    {
        if (IsAsc) {
            return T1.Val3 < T2.Val3;
        } else {
            return T2.Val3 < T1.Val3;
        }
    }
};
...
public:
    ...
    bool operator<(const TQuad& Quad) const
    {
        return (Val1 < Quad.Val1) || ((Val1 == Quad.Val1) && (Val2 < Quad.Val2)) || ((Val1 == Quad.Val1) && (Val2 == Quad.Val2) && (Val3 < Quad.Val3)) || ((Val1 == Quad.Val1) && (Val2 == Quad.Val2) && (Val3 == Quad.Val3) && (Val4 < Quad.Val4));
    }
...
public:
    ...
    int GetPrimHashCd() const
    {
        ...
        for (int i = 0; i < NVals; i++) {
            hc = TPairHashImpl::GetHashCd(hc, ValV[i].GetPrimHashCd());
        }
        return hc;
    }
    int GetSecHashCd() const
    {
        ...
        for (int i = 1; i < NVals; i++) {
            hc = TPairHashImpl::GetHashCd(hc, ValV[i].GetSecHashCd());
        }
        if (NVals > 0) {
            hc = TPairHashImpl::GetHashCd(hc, ValV[0].GetSecHashCd());
        }
        return hc;
    }
...
public:
    ...
    explicit TKeyDat(const TKey& _Key)
        : Key(_Key)
        , Dat()
    {
    }
...
public:
    ...
    {
        if (IsAsc) {
            return P1.Dat < P2.Dat;
        } else {
            return P2.Dat < P1.Dat;
        }
    }
};
...
public:
    ...
    {
        EAssertR(!(IsShM && (MxVals == -1)), "Cannot write to shared memory");
        const TVal Val = ValT[ValN1];
        ValT[ValN1] = ValT[ValN2];
        ValT[ValN2] = Val;
    }
    ...
    {
        TSizeTy SubVals = TSizeTy(EI - BI);
        if (SubVals > TInt::Mx - 1) {
            SubVals = TInt::Mx - 1;
        }
    ...
    {
        AssertR(MxVals != -1, "This vector was obtained from TVecPool. Such vectors cannot change its size!");
        const TSizeTy ValN = SearchForw(Val);
        if (ValN == -1) {
            Add(Val);
            return Last();
        } else {
    ...
    if (ValT == NULL) {
        try {
            ...
            FailR(TStr::Fmt("TVec::Resize: %s, Length:%s, Capacity:%s, New capacity:%s, Type:%s [Program failed to allocate more memory. Solution: Get a bigger machine and a 64-bit compiler.]",
                Ex.what(), TInt::GetStr(Vals).CStr(), TInt::GetStr(MxVals).CStr(), TInt::GetStr(_MxVals).CStr(), GetTypeNm(*this).CStr())
                      .CStr());
        }
    } else {
        TVal* NewValT = NULL;
...
bool TVec<TVal, TSizeTy>::operator<(const TVec<TVal, TSizeTy>& Vec) const
{
    ...
    if (Len() == Vec.Len()) {
        for (TSizeTy ValN = 0; ValN < Vals; ValN++) {
            if (ValT[ValN] < Vec.ValT[ValN]) {
                return true;
            } else if (ValT[ValN] > Vec.ValT[ValN]) {
                return false;
            } else {
            }
        }
...
void TVec<TVal, TSizeTy>::Trunc(const TSizeTy& _Vals)
{
    ...
    if ((_Vals != -1) && (_Vals >= Vals)) {
        return;
    } else if (((_Vals == -1) && (Vals == 0)) || (_Vals == 0)) {
        if (ValT != NULL) {
            delete[] ValT;
        }
        MxVals = Vals = 0;
...
void TVec<TVal, TSizeTy>::Ins(const TSizeTy& ValN, const TVal& Val)
{
    ...
    for (TSizeTy MValN = Vals - 2; MValN >= ValN; MValN--) {
...
TSizeTy TVec<TVal, TSizeTy>::Partition(const TSizeTy& MnLValN, const TSizeTy& MxRValN, const bool& Asc)
{
    ...
    TSizeTy LValN = MnLValN - 1;
    TSizeTy RValN = MxRValN + 1;
    forever
    {
        if (Asc) {
            do {
...
void TVec<TVal, TSizeTy>::Merge()
{
    ...
    AssertR(MxVals != -1, "This vector was obtained from TVecPool. Such vectors cannot change its size!");
    TVec<TVal, TSizeTy> SortedVec(*this);
    SortedVec.Sort();
    Clr();
...
TSizeTy TVec<TVal, TSizeTy>::SearchVForw(const TVec<TVal, TSizeTy>& ValV, const TSizeTy& BValN) const
{
    ...
    for (TSizeTy ValN = BValN; ValN < Vals - ValVLen + 1; ValN++) {
        ...
        for (TSizeTy SubValN = 0; SubValN < ValVLen; SubValN++) {
            if (ValV[SubValN] != GetVal(ValN + SubValN)) {
                Found = false;
                break;
            }
        }
...
public:
    ...
    void GetV(const int& VId, TValV& ValV) const
    {
        if (GetVLen(VId) == 0) {
            ...
            ValV.GenExt(GetValVPt(VId), GetVLen(VId));
        }
    }
    ...
    void PutV(const int& VId, const TValV& ValV)
    {
        ...
        if (FastCopy) {
            ...
            for (::TSize ValN = 0; ValN < ::TSize(ValV.Len()); ValN++, ValPt++) {
                *ValPt = ValV[ValN];
            }
        }
    }
...
template <class TVal, class TSizeTy>
TVecPool<TVal, TSizeTy>::TVecPool(const TSize& ExpectVals, const TSize& _GrowBy, const bool& _FastCopy, const TVal& _EmptyVal)
    : GrowBy(_GrowBy)
    , MxVals(0)
    , Vals(0)
    , EmptyVal(_EmptyVal)
...
template <class TVal, class TSizeTy>
TVecPool<TVal, TSizeTy>::TVecPool(const TVecPool& Pool)
...
void TVecPool<TVal, TSizeTy>::Save(TSOut& SOut) const
{
    ...
    SOut.Save(EmptyVal);
    for (TSize ValN = 0; ValN < Vals; ValN++) {
        ValBf[ValN].Save(SOut);
    }
    {
        SOut.Save(IdToOffV.Len());
        SOut.Save(IdToOffV.Len());
        for (int ValN = 0; ValN < IdToOffV.Len(); ValN++) {
            const uint64 Offset = IdToOffV[ValN];
            SOut.Save(Offset);
        }
    }
}
...
int TVecPool<TVal, TSizeTy>::AddV(const TValV& ValV)
{
    ...
    if (FastCopy) {
        ...
        for (TSize ValN = 0; ValN < ValVLen; ValN++) {
...
void TVecPool<TVal, TSizeTy>::CompactPool(const TVal& DelVal)
{
    ...
    for (int vid = 1; vid < IdToOffV.Len(); vid++) {
        ...
        if (TotalDel > 0) {
            IdToOffV[vid - 1] -= TotalDel;
        } // update end of vector
        if (Len == 0) {
            continue;
        }
        NDel = 0;
...
public:
    ...
    void PutV(const int& VId, const TValV& ValV)
    {
        ...
        if (FastCopy) {
            ...
            TVal* ValPt = GetValVPt(VId);
            for (uint ValN = 0; ValN < uint(ValV.Len()); ValN++, ValPt++) {
                *ValPt = ValV[ValN];
            }
        }
    }
...
void TVecPool<TVal>::Resize(const ::TSize& _MxVals)
{
    ...
    if (ValBf == NULL) {
        ...
        try {
            ...
            FailR(TStr::Fmt("TVecPool::Resize 2: %s, MxVals: %d. [Program failed to allocate more memory. Solution: Get a bigger machine and a 64-bit compiler.]", Ex.what(), _MxVals).CStr());
        }
        IAssert(NewValBf != NULL);
        if (FastCopy) {
            memcpy(NewValBf, ValBf, Vals * sizeof(TVal));
    ...
    , IdToOffV(Pool.IdToOffV)
{
    try {
        ...
        FailR(TStr::Fmt("TVecPool::TVecPool: %s, MxVals: %d", Ex.what(), MxVals).CStr());
    }
    IAssert(ValBf != NULL);
    if (FastCopy) {
        memcpy(ValBf, Pool.ValBf, MxVals * sizeof(TVal));
...
TVecPool<TVal>& TVecPool<TVal>::operator=(const TVecPool& Pool)
{
    if (this != &Pool) {
        ...
        try {
            ...
            FailR(TStr::Fmt("TVec::operator= : %s, MxVals: %d", Ex.what(), MxVals).CStr());
        }
        IAssert(ValBf != NULL);
        if (FastCopy) {
            memcpy(ValBf, Pool.ValBf, Vals * sizeof(TVal));
        } else {
            for (uint64 ValN = 0; ValN < Vals; ValN++) {
                ValBf[ValN] = Pool.ValBf[ValN];
            }
        }
    }
...
{
    for (TSizeTy X = 0; X < XDim - 1; X++) {
        SwapX(X, X + Rnd.GetUniDevInt(XDim - X));
    }
}
...
{
    for (TSizeTy Y = 0; Y < YDim - 1; Y++) {
        SwapY(Y, Y + Rnd.GetUniDevInt(YDim - Y));
    }
}
...
public:
    ...
    {
        return (XDim == Vec.XDim) && (YDim == Vec.YDim) && (ZDim == Vec.ZDim) && (ValV == Vec.ValV);
    }
...
public:
    ...
    int AddNode(const int& ParentNodeId, const TVal& NodeVal = TVal())
    {
        ...
        if (ParentNodeId != -1) {
            NodeV[ParentNodeId].Val2.Add(NodeV.Len());
        }
...
void TTree<TVal>::GetNodeIdV(TIntV& NodeIdV, const int& NodeId)
{
    ...
    } else if (GetParentNodeId(NodeId) == -1) {
        return;
    }
    NodeIdV.Add(NodeId);
    ...
    , LastNd(NULL)
{
    int CheckNds = 0;
    SIn.Load(CheckNds);
    for (int NdN = 0; NdN < CheckNds; NdN++) {
        AddBack(TVal(SIn));
    }
    Assert(Nds == CheckNds);
}
...
TLstNd<TVal>* TLst<TVal>::AddFrontSorted(const TVal& Val, const bool& Asc)
{
    ...
    if (Nd == NULL) {
        ...
        if (Nd == NULL) {
            return Ins(Nd->Last(), Val);
        } else {
            return Ins(Nd->Prev(), Val);
        }
    }
}
...
void TLst<TVal>::PutFront(const PLstNd& Nd)
{
    ...
    if (Nd->PrevNd == NULL) {
        ...
        Nd->PrevNd->NextNd = Nd->NextNd;
    }
    if (Nd->NextNd == NULL) {
        LastNd = Nd->PrevNd;
    } else {
        Nd->NextNd->PrevNd = Nd->PrevNd;
    }
    // add to front
    Nd->PrevNd = NULL;
    Nd->NextNd = FirstNd;
